...
**Live Web Search (V3 + V2):** Before each section's LLM calls, `_fetch_web_context(topic, section_name)` fires two targeted SerpAPI queries (`"{topic} {section_name} {year}"` + `"{topic} {section_name} statistics data {year}"`), deduplicates results by URL, and injects a `## Live Web Research — Retrieved Now` block into the prompt. LLMs cite these injected results as primary sources; any pre-2024 training-data fallback must be marked explicitly. Gracefully degrades to empty string when `SERPAPI_KEY` is absent.
