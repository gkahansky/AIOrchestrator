...
| 2026-05-31 14:00 |  | TBD     | Content Engine M1 — new venture for multi-channel content creation + publishing: brands/strategies/items/assets/social_accounts/publish_jobs models + migration, publishers registry (LinkedIn/FB/IG/YouTube with assisted-send fallback), AI-tell critic + banned-phrase quality gate, FastAPI router, Celery tasks + 5-min publish scheduler beat, EchoForge Accessibility brand seed, planBadmin Content Engine tab |
| 2026-05-31 16:00 |  | TBD     | Content Engine M2 — OAuth flows (LinkedIn / Meta / YouTube) with signed-state CSRF protection, image upload on all four publishers (LinkedIn assets register-upload, FB Page multipart + carousel attached_media, IG container 2-step + 3-step carousel + reels), public asset endpoint for IG, token refresh helper, frontend Connect buttons + post-callback toast |
