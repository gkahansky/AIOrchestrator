...
| 2026-05-23 00:10 |  | (merge) | Merge campaign-publication-workflow to main; document post-creation schedule editing in root CLAUDE.md outreach section |
| 2026-05-24 00:00 |  | 1284ed0 | Let users define search sources during campaign creation — replaced the outreach-platform dropdown with an Add Source control, require ≥1 source before saving, and accept the source list on the create endpoint (API still falls back to venture defaults) |
| 2026-05-24 00:30 |  | 545d5b1 | Align outreach platform with lead source for both message style and delivery — SOURCE_TO_OUTREACH + resolve_effective_platform() in compose_personalized.py; collapses to email until a platform is added to DELIVERABLE_PLATFORMS; campaign default platform seeded from primary source |
| 2026-05-25 00:00 |  | ac352e7 | Campaign publication workflow — reusable persona library (personas + campaign_personas many-to-many, JSONB backfill), auto-compose suggestion draft at find time, pluggable send-handler registry (email via Resend + assisted send for linkedin/instagram/facebook with provider slot), single-lead send + assisted-send confirm endpoints, persona CRUD/link endpoints, frontend Personas tab + per-lead send/confirm UX; made outreach_sends.template_id nullable |
| 2026-05-25 00:20 |  | b9d030c | Documented the campaign publication workflow in the root CLAUDE.md cold-outreach section — persona library, auto-compose, send-handler registry, single/assisted send endpoints, and the awaiting_send review gate |
| 2026-05-25 00:30 |  | (merge) | Merge campaign-publication-workflow to main — reusable persona library, auto-compose at find time, pluggable send-handler registry (email + assisted social send), single/assisted send endpoints |
| 2026-05-25 01:00 |  | 43e535e | Marketing UI overhaul — campaigns table with clickable rows + view/edit popup (reusable CampaignForm), standalone Personas page with deep-linkable persona chips, Contacts "Create contact" + POST /contacts endpoint; list_campaigns returns sources; seed_personas.py for accessibility personas |
| 2026-05-25 01:10 |  | (merge) | Merge campaign-publication-workflow to main — Marketing campaigns table + view/edit popup, standalone Personas page, Contacts create flow |
| 2026-05-25 01:30 |  | bcda48d | Marketing UI fixes — contacts URL column + edit-contact modal, campaign persona input switched to a library selector with inline "Add persona", widened campaign popup, and collapsible (two-line) persona descriptions on the Personas page |
...
| 2026-05-27 11:00 |  | fdf4821 | Add "Revise response" — POST /drafts/{id}/revise regenerates a draft from operator feedback (rewrites grounded in the lead's post, returns to pending_review); DraftCard gets a Revise button + feedback box on pending_review and rejected drafts |
| 2026-05-27 13:00 |  | eb4b5d0 | Upgrade social lead search — RawPost captures author profile URL/handle + recency/engagement/role signals (new leads.author_url col, migration l7m8n9o0p1q2; assisted send deep-links the profile); search-side provider slot mirroring the send side; query_builder + relevance filters prune before Haiku; Facebook group discovery + comment mining; Apify sync endpoint with Redis result cache + daily cost cap; first source-handler tests |
