...
| 2026-06-02 09:00 |  | TBD     | Content Engine M4 — carousel-aware image prompt builder (`media/carousel_brief.py`, single style anchor across N slides), brand-voice regeneration endpoint (re-scrape source URLs via `_fetch_page` + run `generate_brand_voice`), per-brand AI-tell auto-approve threshold (item skips review_pending when score ≥ threshold), weekly cost-per-published-post digest beat (`content.run_weekly_cost_digest`). Adds 2 brand fields + migration `n9o0p1q2r3s4`. BrandIn split into BrandIn/BrandPatch for partial updates. Frontend BrandCard with threshold slider + regenerate-voice button. CI test_tool_router stale assertions fixed (Gemini Imagen is the active standard). |
