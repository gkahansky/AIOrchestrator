...
| 2026-06-03 14:00 |  | TBD     | Image-gen quota failover — `generate_image` now auto-falls-back to the DALL-E 3 fallback tier when Gemini Imagen returns 429/RESOURCE_EXHAUSTED. Result carries `failover_from`/`failover_reason` so the operator sees the degradation in the item. Video pipeline (`generate_video_explainer`) wraps each cue in try/except; on `kind=generated` failure derives a Pexels stock query so the video ships with valid b-roll instead of crashing. Failed cues collected in `visual_failures` and persisted on the asset. |
