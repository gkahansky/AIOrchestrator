...
| U-03 | Project Management Integration | System | `planned` | Integrate platform tasks into Jira or Monday.com. Track what Claude is doing, what Guy is doing, and what 3rd parties own. Board columns: Backlog → In Progress → Review → Done. Each pipeline phase and sprint task maps to a card. Skill: `comms/update_project_board.py`. Evaluate Monday API vs Jira Cloud API — prefer whichever has simpler OAuth + card-creation endpoint. |
...
| H-02 | Marketing Audit — Sprint 2: PDF Polish | Venture | `in-progress` | Sample PDF fully polished (Echoforge branding, correct margins, centred score pill, accurate redaction counts, mailto CTA). Remaining: brand the full PDF (logo, score gauges, bar charts) + build `audit_deliver.py` (Drive upload + marketplace delivery). Venture: `marketing_audit`. |
...
| H-07 | Gemini Imagen 4 Integration | System | `done` | Gemini Imagen 4 (`imagen-4.0-generate-001`) live as standard-tier image tool. DALL-E 3 demoted to fallback. Mockup skill (`create_mockup.py`) fully rewritten — Imagen 4 Fast generates 3 photorealistic room scenes end-to-end. Pillow composite retained as fallback. Tested end-to-end: artwork + 3 mockups generated successfully. |
...
| M-03 | Podcast Add-on: Brand Voice Guide | Venture | `planned` | Analyse 2–3 episode transcripts → structured Brand Voice Guide (tone, vocabulary, sentence style). Cached as JSON per client for reuse. Skill: `generate_brand_voice.py`. Price: $79. |
| M-04 | Podcast Add-on: Episode Promotional Copy | Venture | `planned` | Platform description + audiogram caption + newsletter teaser + LinkedIn post from one episode. Skill: `generate_promo_copy.py`. Price: $39. |
...
| D-11 | Gemini Imagen 4 Integration | System | 2026-03 | Gemini Imagen 4 (`imagen-4.0-generate-001`) live as standard-tier artwork tool; DALL-E 3 demoted to fallback. `create_mockup.py` rewritten: Imagen 4 Fast generates 3 photorealistic room scenes end-to-end. Pillow composite retained as fallback. Tested: artwork + 3 mockups generated. Skills: `media/generate_image.py`, `media/create_mockup.py`, `registry/skills.json`. |
