...
| D-20 | Drive Auth — User OAuth Over Service Account | System | 2026-04-05 | `_drive_auth.py` already prefers user OAuth token (`GOOGLE_TOKEN_PATH`) over service account. `main.py` already materialises `GOOGLE_DRIVE_TOKEN_JSON` env var to `/tmp/google_token.json` on startup. Fix is Railway configuration only: set `GOOGLE_DRIVE_TOKEN_JSON` (base64-encoded `google_token.json`) + `GOOGLE_TOKEN_PATH=/tmp/google_token.json` on both web and worker services to use personal 5TB Drive quota instead of the service account's 15GB limit. |
