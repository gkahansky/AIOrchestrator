...
import { fetchPodcastOrders, createPodcastOrder, createCRJob, fetchCRJobs } from "../../api"
import StatusBadge from "../../components/StatusBadge"
import PhaseBar from "../../components/PhaseBar"
import JobLinks from "../../components/JobLinks"

type Tab = "overview" | "new_order" | "cr_order" | "orders" | "cr_orders"
...
function CROrderForm() {
  const navigate = useNavigate()
  const [plan, setPlan] = useState<string>("starter")
  const [driveVideoId, setDriveVideoId] = useState("")
  const [videoSuffix, setVideoSuffix] = useState(".mp4")
  const [showName, setShowName] = useState("")
  const [episodeTitle, setEpisodeTitle] = useState("")
  const [hostName, setHostName] = useState("")
  const [guestName, setGuestName] = useState("")
    ...
    <form onSubmit={handleSubmit} className="max-w-xl space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-on-surface-variant/60 mt-1 font-label">
          Found in the Drive share URL after <code>/d/</code>. Upload the video to Drive first.
        </p>
      ...
      {mutation.isSuccess && (
        <div className="bg-secondary-container text-on-secondary-container rounded-xl px-4 py-3 text-sm font-label">
          Job queued — redirecting to jobs list…
        </div>
      )}
...
function CROrdersList() {
  ...
  if (error) return (
    <div className="bg-error-container text-on-error-container rounded-xl px-4 py-3 text-sm font-label">
      Failed to load CR jobs: {(error as Error).message}
    </div>
  )
...
export default function ContentStudio() {
  ...
  const tabs: { id: Tab; label: string }[] = [
    ...
    { id: "new_order", label: "Show Notes Order" },
    { id: "orders",    label: "Show Notes Jobs" },
    { id: "cr_order",  label: "Repurposing Order" },
    { id: "cr_orders", label: "Repurposing Jobs" },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm font-body text-on-surface-variant">
            EchoForge — Podcast Show Notes &amp; Content Repurposing
          </p>
        </div>
      ...
      <div className="flex gap-0.5 bg-surface-container-low p-1 rounded-xl w-fit flex-wrap">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
