...
async def run_accessibility_scan(url: str, timeout_ms: int = 30000) -> dict:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # results object contains multiple attributes like `violations`, `passes`, `incomplete`, etc.
            ...
            def parse_result_nodes(rule_list):
                ...
                for rule in rule_list:
                    parsed.append({
                        "id": rule.id,
                        "impact": getattr(rule, "impact", None),
                        "tags": rule.tags,
                        ...
                        "nodes": [{
                            ...
                            "html": node.html,
                            "impact": node.impact,
                            "failureSummary": getattr(node, "failureSummary", None)
                        } for node in rule.nodes]
                    })
                return parsed

            violations = parse_result_nodes(results.violations)
            passes = parse_result_nodes(results.passes)
            incomplete = parse_result_nodes(results.incomplete)
