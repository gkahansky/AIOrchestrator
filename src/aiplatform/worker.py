...
def deliver_accessibility_audit_job(job_id: str, review_notes: str | None = None) -> dict:
    ...
    try:
        ...
        audit = db.query(AccessibilityAudit).filter(AccessibilityAudit.job_id == parsed_job_id).first()
        current_output = dict(job.output_data or {})
        ...
        client_email = current_output.get("client_email") or current_input.get("client_email") or current_input.get("client_id")
        ...
        if client_email:
            ...
            send_result = send_email(
                ...
                body_html=(
                    f"<p>Your accessibility audit for <strong>{current_input.get('url', current_output.get('url', 'your site'))}</strong> is ready.</p>"
                    f"<p>Open the report here: <a href=\"{report_link}\">View accessibility report</a></p>"
                    f"<p>If you have any questions about the findings or remediation priorities, reply to this email.</p>"
                ),
            )
