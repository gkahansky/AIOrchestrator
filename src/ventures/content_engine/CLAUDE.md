...
| CE-18 | Brand edit modal (banned phrases / voice URLs / personas / cadence / theme weights) | ✅ done | Replaces inline brand widgets with a full edit form. Chip editor for banned phrases + voice sources; JSON editors for personas / themes / cadence. | 2026-06-03 |
| CE-19 | Strategy draft form — operator brief + tone notes + must-cover topics | ✅ done | Modal pre-draft form. `user_brief`, `must_cover_topics`, `upcoming_events`, `tone_notes` flow into `generate_calendar()` → Claude enrichment prompt. Full-page spinner while drafting. | 2026-06-03 |
| CE-20 | Editable calendar — date pickers, channel/format/pillar dropdowns, headers, add/remove rows | ✅ done | Replaces the read-only calendar view. Format dropdown filters by channel's allow-list. Saves via `PATCH /strategies/{id}`. | 2026-06-03 |
| CE-21 | Manual social-account form (paste token, multi-account per channel) | ✅ done | "+ Add manually" button per brand. Surfaces all accounts grouped by platform so 2+ LinkedIn pages per brand are visible. Per-platform hint text (URN format / Page ID / IG user ID). | 2026-06-03 |
...
| CE-23 | Image-gen quota failover (Imagen 429 → DALL-E 3) | ✅ done | `generate_image` auto-retries through the `fallback` tier on RESOURCE_EXHAUSTED / 429 / rate-limit detected via shared `_is_quota_error`. Result carries `failover_from` + `failover_reason`; pipeline surfaces it in `item.error_message` so the operator sees the degradation. | 2026-06-03 |
