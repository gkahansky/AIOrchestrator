...
_HEADERS = {
    ...
    "Accept-Language": "en-US,en;q=0.9,he;q=0.3",
...
def fetch_page(url, timeout=PAGE_TIMEOUT):
    ...
    1. urllib  — fast, no extra deps.
    2. requests — handles persistent sessions, cookies, and redirect chains
       that urllib drops, which passes basic bot-detection on many sites.
    """
    # Attempt 1: urllib
    ...
    # Attempt 2: requests (better session/cookie/redirect handling)
    try:
        import requests as _req
        import warnings
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress InsecureRequestWarning
            resp = _req.get(url, headers=_HEADERS, timeout=timeout, verify=False, allow_redirects=True)
